...
def setup_tracer(app: FastAPI) -> None:
    ...
    FastAPIInstrumentor().instrument_app(
        app,
        excluded_urls="api/.*/healthcheck,openapi",
...
def server_request_hook(span: Span, scope: dict):
    if span and span.is_recording():
        if request_id := next(filter(lambda x: x[0].decode() == get_settings().jaeger.header, scope["headers"]), None):
            span.set_attribute("http.request_id", request_id[1].decode())
        else:
