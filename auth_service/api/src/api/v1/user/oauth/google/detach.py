...
class DetachGoogleAccountView(SwaggerView):
    decorators = [
        extract_jwt,
        extract_connect_info,
    ]
    summary = "Detach Google account"
    description = "Detach Google account from user"
    tags = ["oauth"]
    security = [{"bearerAuth": []}]
    ...
    responses = {
        HTTPStatus.NO_CONTENT.value: {
            "description": "Google account has been successfully detached",
        },
        HTTPStatus.NOT_FOUND.value: {
            "description": "Google account was not attached",
        },
    ...
    def delete(self, token: str, connect_info: ConnectionInfo) -> Response:
        """
        Detach Google account from user.
        ---
        """
        answer = get_oauth_google_service().detach_account(
            CommonRequest(
